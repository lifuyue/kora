...
@Singleton
class RoomBackedKnowledgeRepository @Inject constructor(
    ...
    override fun observeCollections(datasetId: String): Flow<List<CollectionListItemUiModel>> =
        cachedCollectionDao.observeCollectionsForDataset(datasetId).combine(importTaskDao.observeTasksForDataset(datasetId)) { items, tasks ->
...
private fun String.extractField(name: String): String =
    Regex("\"$name\":\"(.*?)\"").find(this)?.groupValues?.getOrNull(1).orEmpty()
