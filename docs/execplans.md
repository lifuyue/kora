# Plan: Kora 产品完整里程碑与 Codex 逐步实现计划

## Context

Kora 项目目前状态：
- **文档体系已就位**：73 个 spec 文件（skeleton 级别），CLAUDE.md、INDEX.md、GLOSSARY.md、CONVENTIONS.md 已创建
- **参考仓库已克隆**：`.reference/FastGPT`（v4.14.9.5）和 `.reference/open-webui`（v0.8.12）
- **无源码**：项目仍是纯文档 + LICENSE + README，无 Android 工程

现在需要：将 skeleton docs 充实为 **source-referenced 精确规格**，然后从零搭建 Android 工程，分阶段由 coding agent 实现完整产品。
