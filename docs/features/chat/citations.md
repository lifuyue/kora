...
Open WebUI 把引用当作聊天阅读体验的一部分，FastGPT 又把引用与检索调试能力连成闭环，因此 Kora 需要同时保留消息内引用入口和知识库侧检索测试，而不是只实现其中一侧。参考 [../../reference/open-webui-implementation-patterns.md](../../reference/open-webui-implementation-patterns.md) 和 [../../reference/fastgpt-implementation-patterns.md](../../reference/fastgpt-implementation-patterns.md)。
