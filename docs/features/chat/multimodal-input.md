...
## Overview
输入区能力参考 Open WebUI `MessageInput/FilesOverlay` 等组件，但针对 Android 采用系统 picker + 本地预览。目标是把图片、文件、视频、音频和自定义扩展名统一挂到同一个发送草稿模型上。

## Functional Requirements
- [ ] FR-1: 支持从系统文件选择器和图片选择器添加附件。
